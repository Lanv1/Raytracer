...
public:
    ...
    void draw() const {
        ...
        // std::cout<<"diffuse; "<<material.diffuse_material<<std::endl;
        // std::cout<<"spec; "<<material.specular_material<<std::endl;
        // std::cout<<"ambiant; "<<material.ambient_material<<std::endl;
    ...
    virtual RayIntersection intersect( Ray const & ray ) const {
        ...
        float triangleScaling = 1.00001;
        for(int i = 0; i < size; i ++) {
            MeshTriangle triIndex = triangles[i];
            
            tri_valid = triIndex.v[0] != triIndex.v[1] && triIndex.v[2] != triIndex.v[1] && triIndex.v[0] != triIndex.v[2];    

            if(tri_valid){
                Triangle tri = Triangle(triangleScaling * vertices[triIndex[0]].position, triangleScaling * vertices[triIndex[1]].position, triangleScaling * vertices[triIndex[2]].position);
                
                currentIntersection = tri.getIntersection(ray);

                if(currentIntersection.intersectionExists){

                    if(currentIntersection.t < closestIntersection.t){
                        closestIntersection.t = currentIntersection.t;
                        closestIntersection.objType = currentIntersection.objType;
                        closestIntersection.intersectionExists = true;
                        closestIntersection.w0 = currentIntersection.w0;
                        closestIntersection.w1 = currentIntersection.w1;
                        closestIntersection.w2 = currentIntersection.w2;
                        ...
                        interpN.normalize();
                        closestIntersection.normal = interpN;
                    }
                }
